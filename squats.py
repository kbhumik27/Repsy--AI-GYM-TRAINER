...
# Function to check squats form
def check_squats_form(hip, knee, ankle):
    hip_knee_angle = calculate_angle(hip, knee, ankle)

    if hip_knee_angle < 140:  # Knees are not fully extended
        return "Straighten your legs"
    elif hip_knee_angle > 170:  # Knees are too far forward
        return "Keep your knees behind your toes"
    else:
        return "Great form!"

# Main function for squats
def squats():
    ...
    while True:
        ...
        try:
            ...
            form_feedback = check_squats_form(hip, knee, ankle)
            cv2.putText(image, form_feedback, (10, 60),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (140, 255, 255), 2, cv2.LINE_AA)
